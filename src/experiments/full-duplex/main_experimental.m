...
%% Common Parameters
pCHDownlink = 'software';           % Channel can be: 'software', or 'adalm2000'
pCHUplink = 'software';             % Channel can be: 'software', or 'adalm-pluto'
...
gNBConfig.DCIModulation = 20;             % DCI Modulation:16QAM coderate: 434/1024
...
pSimArg.pSNRdB = -1;                           % SNR for AWGN Channel in dB
pSimArg.pFrequencyOffset = 1.2e3;               % Frequency offset (Hz)
pSimArg.pPhaseOffset = 15;                      % Phase offset (Degrees)

%% ADALM PLUTO Parameters
pPlutoArg.pGain = -10;                          % Transmitter Gain in dB
...
pM2K.pVLCCenterFrequency = 3e6;          % VLC Center Frequency
...
berUL = 0;              % Bit error rate in Uplink Transmission
...
while (cont < pMaxIt)
    ...
    if strcmp(pCHUplink,'software') && strcmp(pCHDownlink,'software') 
        ...
        fullDLTx = [fullDLTx;reshape(gNBtrBlk,1,[])];
        fullDLRx  = [fullDLRx ;reshape(UErxBit,1,[])];
        fullULTx = [fullULTx ; reshape(UEtrBlk,1,[])];
        fullULRx = [fullULRx ; reshape(gNBrxBit,1,[])];

        % Concatenate bits transferred without CR
        fullDLTxnoCRC = [auxDLTxnoCRC;reshape(gNBCodedBlk,1,[])];
        fullDLRxnoCRC  = [auxDLRxnoCRC ;reshape(UErxRawBit,1,[])];
        fullULTxnoCRC = [auxULTxnoCRC ; reshape(UECodedBlk,1,[])];
        fullULRxnoCRC = [auxULRxnoCRC ; reshape(gNBRxRawBit,1,[])];
    ...
    if (evmULcur < 7.5 && strcmp(pCHUplink,'adalm-pluto') && evDLcur < 10 && strcmp(pCHDownlink,'adalm2000') )
        cont = cont + 1;
    ...
    updateAnnotDashboard2(berDLnoCRC,berULnoCRC);
